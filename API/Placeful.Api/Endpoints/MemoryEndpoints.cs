...
public static class MemoryEndpoints
{
    public static void MapMemoryEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("", GetMemories).WithName(nameof(GetMemories));
        group.MapGet("{memoryId:guid}", GetMemory).WithName(nameof(GetMemory));
        group.MapPost("", CreateMemory).WithName(nameof(CreateMemory));
        group.MapPut("", UpdateMemory).WithName(nameof(UpdateMemory));
        group.MapDelete("{memoryId:guid}", DeleteMemory).WithName(nameof(DeleteMemory));
    }
    
    private static async Task<IResult> GetMemories(IMemoryService memoryService, IMapper mapper)
    {
